...
-- Snapshot saved to supabase/sql_history/20260222_money_ledger_recalc.sql

-- Event types: revenue/cost classification for recalc (idempotent)
DO $$
BEGIN
...
-- Immutable ledger: one row per financial fact
...
-- Idempotency: (account_id, source, external_id, event_type) unique for safe re-imports
...
-- No UPDATE/DELETE: immutable ledger (or restrict to system corrections only later)

-- Item financial summary (recalc): derived from money_events per inventory_item_id
...
-- Derived totals for item: gross, net, profit, ROI
...
-- Sale financial summary: by sale_id (includes sale-level and line-level events)
...
-- Emit money_events when a sale is created (sale-level fees, taxes, shipping)
...
-- Emit money_events when a sale_line_item is inserted (SOLD_REVENUE, COST_BASIS per line)
...
Fixing the SQL file: removing the duplicate view definition and correcting trigger conflict handling.
